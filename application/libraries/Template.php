 ...
 *
 * Made compatible with CodeIgniter and added features like: 
 * - Control error display.
 * - Case Insensitive variables and blocks.
 * - Add variables and blocks with arrays.
 * 
 * @author Paulo Lima <paulo.lima@valkhan.com) 
 * @version 2.3
 */
class Template {
    ...
    /**
     * Defines if an error should stop the script with exceptions.
     * loadFile() will trigger error even if set to false when file is not found!
     * @var  boolean
     */
    private $dieOnError = true;

    /**
     * Defines if class will accept case insensitive vars/blocks when getting values.
     * It's a good practice to leave vars and blocks in upper case on template strings/files.
     * @var boolean
     */
    private $caseInsensitiveVars = true;
     ...
     *
     * @param     array   $params		Compatibility with CI autoload config 
     */
    public function __construct($params = array()) {
        $this->dieOnError = isset($params['dieonerror']) ? $params['dieonerror'] : $this->dieOnError;
        $this->caseInsensitiveVars = isset($params['caseinsensitivevars']) ? $params['caseinsensitivevars'] : $this->caseInsensitiveVars;
        $this->accurate = isset($params['accurate']) ? $params['accurate'] : $this->accurate;
    ...
    public function addFile($varname, $filename) {
        if($this->caseInsensitiveVars)
            $varname = strtoupper($varname);
        if (!$this->exists($varname) && $varname != '.') {
            if ($this->dieOnError)
    ...
    /**
     * Adds a block from an array. Example:
     * array(
     *     array(
     *         'var1' => 'value1', 'var2' => 'value2', 
     *     )
     * )
     * @param string $bloco Block name
    ...
    /**
     * Adds variables from an array. Example:
     * array(
     *   'var1' => 'value1', 'var2' => 'value2',   
     * )
     * @param array $data 
    ...
    /**
     * Adds an array of variables or blocks. 
     * @param array $data Array of variables
     * @param string|boolean $bloco Default = false (no blocks) or string name of the block to add the $data
     * @return \Template
     */
    ...
    /**
     * Setter for DieOnError
     * @param boolean $die true = will stop script 
     * @return \Template
     */
    public function setDieOnError($die) {
        $this->dieOnError = $die ? true : false;
        return $this;
    }

    /**
     * Verify if a block exists
